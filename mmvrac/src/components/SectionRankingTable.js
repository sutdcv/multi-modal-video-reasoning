...
const SectionRankingTable = (props) => {
    const get_attempts_left = (team) => {
        if (team.attempts_left >= 0) {
            return team.attempts_left
        } else {
            return "-"
        }
    }

    const get_number_color_class = (team) => {
        if (team.attempts_left >= 5) {
            return "bg-secondary"
        } else if (team.attempts_left === 1) {
            return "bg-warning"
        } else if (team.attempts_left === 0) {
            return "bg-danger"
        } else {
            return "bg-info"
        }
            ...
            <table className="table table-light table-striped table-hover table-bordered">
                <thead>
                    <tr className="d-flex user-select-none">
                        ...
                        <th className="col-7" scope="col"><span className="badge rounded-pill bg-secondary">Team Size</span> | Organization(s)</th>
                ...
                <tbody>
                    {
                        ...
                        <tr className="d-flex" key={index + 1}>
                            ...
                            <td className="col-7 text-capitalize"><span className="badge rounded-pill bg-secondary user-select-none">{team.team_size}</span> | {team.team_org}</td>
                            <td className="col-2 text-center user-select-none"><span className={"badge rounded-pill user-select-none " + get_number_color_class(team)}>{get_attempts_left(team)}</span></td>
